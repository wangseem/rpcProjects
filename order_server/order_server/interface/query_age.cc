...
QueryAgeInterface::QueryAgeInterface(const ::queryAgeReq* request, ::queryAgeRes* response, 
...
QueryAgeInterface::~QueryAgeInterface() {
  APPINFOLOG("Out|response:{%s}", m_response->ShortDebugString().c_str());
}

void QueryAgeInterface::run() {
...
void QueryAgeInterface::setError(int code, const std::string& err_info) {
